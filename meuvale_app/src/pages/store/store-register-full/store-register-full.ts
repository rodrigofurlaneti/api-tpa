...
export class StoreRegisterFullPage {
  ...
  { Id: 3, Descricao: "POSWEB", Ativo: false },
  ...
  search(event: any) {
    ...
    this.habilitaBotaoAceitar = cnpj != "" && cnpj.length == 18;
  }
  ...
  private machineFormToUserData(): any {
    ...
    let machineLoja: TerminaisLoja = {
      ...
      Terminal: {
        ...
        TaxaRefeicao: Number.parseFloat(_formData.refeicao.replace(',', '.')),
        TaxaAlimentacao: Number.parseFloat(_formData.alimentacao.replace(',', '.')),
        TaxaCombustivel: Number.parseFloat(_formData.combustivel.replace(',', '.')),
        TaxaAdiantamento: Number.parseFloat(_formData.adiantamento.replace(',', '.'))
      }
    };
      ...
      this.machineForm.setValue(
        {
          ...
          refeicao: data ? data.Terminal.TaxaRefeicao.toFixed(2) : 0,
          alimentacao: data ? data.Terminal.TaxaAlimentacao.toFixed(2) : 0,
          combustivel: data ? data.Terminal.TaxaCombustivel.toFixed(2) : 0,
          adiantamento: data ? data.Terminal.TaxaAdiantamento.toFixed(2) : 0
        }
      )
    }
  }
